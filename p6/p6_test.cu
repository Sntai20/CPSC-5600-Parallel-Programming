...
const string input_filename_test = "p6/out/test/x_y_16.csv";
const string output_filename_test = "p6/out/test/x_y_scan_16.csv";
...
bool file_utils_read_csv_test(string input_filename) {
    ...
    if (data.size() != 16) {
        cout << "Expected 16 rows, got " << data.size() << endl;
        return false;
    }
